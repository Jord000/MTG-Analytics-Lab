...
until curl -s http://opensearch:9200/_cluster/health | grep -q '"status"'
do
  sleep 2
done
...
if [ "$STATUS" = "404" ]; then
  ...
  curl -X PUT http://opensearch:9200/mtg-game-entries \
...
curl -X POST http://opensearch:9200/mtg-game-entries/_bulk \
  -H "Content-Type: application/json" \
